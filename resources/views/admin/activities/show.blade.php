...
@section('title', 'Detail Kegiatan - ' . $activity->judul_kegiatan)
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">{{ $activity->judul_kegiatan }}</h2>
                
                @if($activity->tujuan_kegiatan)
                <div class="mb-4">
                    <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">Tujuan</h3>
                    <p class="text-gray-600">{{ $activity->tujuan_kegiatan }}</p>
                </div>
                @endif
                ...
                <div>
                    ...
                    <p class="text-gray-600 whitespace-pre-line">{{ $activity->deskripsi }}</p>
                </div>
                @else
            ...
            <!-- Kepanitiaan -->
            @php
                $divisiList = $activity->divisi;
                if (is_string($divisiList)) {
                    $divisiList = json_decode($divisiList, true) ?? [];
                }
                if (!is_array($divisiList)) {
                    $divisiList = [];
                }
            @endphp
            @if($activity->ketua_pelaksana || count($divisiList) > 0)
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-6">
            <!-- Info Card -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4">Informasi Kegiatan</h3>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">{{ $activity->tanggal_kegiatan->translatedFormat('d F Y') }}</p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div class="space-y-3 text-sm">
                    <div class="flex items-center gap-2 text-gray-500">
                        ...
                        <span>Dibuat: {{ $activity->created_at->translatedFormat('d M Y, H:i') }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-gray-500">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                        </svg>
                        <span>Diperbarui: {{ $activity->updated_at->translatedFormat('d M Y, H:i') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <form action="{{ route('admin.activities.toggle-publish', $activity) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" 
                            class="w-full flex items-center justify-center gap-2 px-4 py-2.5 {{ $activity->is_published ? 'bg-gray-100 hover:bg-gray-200 text-gray-700' : 'bg-green-100 hover:bg-green-200 text-green-700' }} font-semibold rounded-xl transition-all">
                            @if($activity->is_published)
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form action="{{ route('admin.activities.destroy', $activity) }}" method="POST" 
                          onsubmit="return confirm('Yakin ingin menghapus kegiatan ini? Aksi ini tidak dapat dibatalkan.')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
